    ...
    try:
        ...
        for frame_num in range(frame_step,int(vidcap.get(cv2.CAP_PROP_FRAME_COUNT)),frame_step):
            try:
                ...
                RMSE_val.append(sum(sum(sum((frame_tens_fixed - img_tens)**2))).numpy()**0.5)
                RMSE_val.append(sum(sum(sum((frame_tens_fixed - img_tens_flipped)**2))).numpy()**0.5)
...
flipped = ["no","yes"]*int(len(RMSE_val)/2)
im_name = [img_list[im_num]]*int(len(RMSE_val))
