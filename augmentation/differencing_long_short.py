...
parser.add_argument("-d2","--diff2", help="2nd Distance in frames between differenced frames and center frame (defaults to 0 - which means it isnt used).",default=1,type=int)
parser.add_argument("-bn","--BS_num", help="Number of frames to base background subtraction average on.",default=1,type=int)
parser.add_argument("-bs","--BS_skip", help="Distance (in frames) between frames to base background subtraction average on.",default=1,type=int)
...
parser.add_argument("-p","--pca", help="Whether to undergo PCA with whitening on the three RGB frames to reduce to two RGB frames or pca on everything to reduce to three frames. Defaults to all.",default="none")
...
for im_num in im_num_list:
    ...
    #        if(BS_count == 0):
    #            frame_empty = torch.empty((BS_num,)+frame_BS_tens.shape)
    #            frame_empty[BS_count] = frame_BS_tens
    #        else:
    #            frame_empty[BS_count] = frame_BS_tens  
            if(BS_count == 0):
    ...
    #        frame_BS_tens_avg = frame_BS_tens_total/BS_count
        ...
        #frame_BS_tens_avg = torch.quantile(frame_empty[0:BS_count], q=0.5,dim=0)    
        #frame_BS_tens_avg = torch.median(frame_empty[0:BS_count],dim=0).values    
        #frame_BS_tens_avg = torch.mean(frame_empty[0:BS_count],dim=0)    
    ...
    if(diff2!=0):
        frame3 = val_frame_dat['frame'][im_num] + diff2
        vidcap.set(1, frame3)
        success,image3 = vidcap.read()
        frame4 = val_frame_dat['frame'][im_num] - diff2
        vidcap.set(1, frame4)
        success,image4 = vidcap.read()
    ...
    if(diff2!=0):
        img_right_tens_far = convert_tensor(image3)
        img_left_tens_far = convert_tensor(image4)
    ...
    if(diff2!=0):
        img_right_tens_far_fixed = torch.stack((img_right_tens_far[2,:,:],img_right_tens_far[1,:,:],img_right_tens_far[0,:,:]),dim=0)
        if(image_size!=False):    
            img_right_tens_far_fixed = convert_tensor(convert_image(img_right_tens_far_fixed).resize(image_size))
        img_left_tens_far_fixed = torch.stack((img_left_tens_far[2,:,:],img_left_tens_far[1,:,:],img_left_tens_far[0,:,:]),dim=0)
        if(image_size!=False):     
            img_left_tens_far_fixed = convert_tensor(convert_image(img_left_tens_far_fixed).resize(image_size)) 
    ...
    if(method=="abs"):
        ...
        diff_im = alpha*0.25*(2*img_right_tens_fixed -img_center_tens-img_left_tens_fixed)  + 0.5
    elif(method=="diff"):
        diff_im = alpha*0.5*(img_center_tens-img_left_tens_fixed)+0.5
    elif(method=="diff_alt"):
        diff_im = alpha*0.5*(img_center_tens-img_left_tens_fixed)
    elif(method=="diff_long_short"):
        diff_im = alpha*0.25*(2*img_center_tens-img_left_tens_fixed - img_left_tens_far_fixed)+0.5
    elif(method=="naive_long_short"):
        diff_im = alpha*(1/6)*(2*img_right_tens_fixed - 2*img_center_tens-img_left_tens_fixed + img_right_tens_far_fixed)+0.5
    elif(method=="naive_BS_short"):
        diff_im = alpha*(1/6)*(2*img_center_tens+img_right_tens_fixed-2*img_left_tens_fixed - frame_BS_tens_avg_fixed)+0.5
    elif(method=="diff_BS_short"):
        diff_im = alpha*0.25*(2*img_center_tens-img_left_tens_fixed - frame_BS_tens_avg_fixed)+0.5
    elif(method=="diff_BS"):
        diff_im = alpha*0.5*(img_center_tens - frame_BS_tens_avg_fixed)+0.5
    else:
    ...
    if(red_layer==True):
        ...
        if(do_pca=="two"):
            ...
            img_center_tens_big = torch.stack((diff_im_avg,img_center_tens[0,:,:],img_center_tens[1,:,:],img_center_tens[2,:,:]),dim=0)
            
            im_depth, im_height, im_width,  = img_center_tens_big.shape

            # Reshape the image tensor into a 2D matrix
            im_matrix = np.reshape(convert_image(img_center_tens_big), (im_height * im_width, im_depth))
            ...
            # Reshape the reduced matrix back into a 3D tensor
            reduced_image = convert_tensor(np.reshape(reduced_matrix, (im_height,im_width, 3)))

            #apply sigmoid transform
            reduced_image = torch.sigmoid(reduced_image)
            
            diff_im_final = reduced_image
        elif(do_pca=="two_alt"):
            if(im_num==0):
