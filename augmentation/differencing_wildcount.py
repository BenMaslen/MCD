...
parser.add_argument("-b","--buffer", help="Thickness in pixels from top and bottom of image that we are setting to zero.",default=60,type=int)
...
pest_monitoring_locator = '../../data/Wildcount/location_data/pest_monitoring_images.csv'
...
pest_monitoring_loc_dat = pd.read_csv(pest_monitoring_locator)
...
for im_num in im_num_list:
        #get image folder
    try:
        if(val_frame_dat['image_folder'][im_num]=='../../data/Wildcount/surrounding_images/pest_monitoring/'):
            img_folder = list(pest_monitoring_loc_dat['image_name'])
        else:
        ...
        #open images and put in a catch if left or right image is not available
        try:
            left_image = Image.open(val_frame_dat['image_folder'][im_num] + left_image_name)
        except:
            left_image =  Image.open(val_frame_dat['image_folder'][im_num] + right_image_name)
        
        try:
            right_image = Image.open(val_frame_dat['image_folder'][im_num] + right_image_name)
        except:
            right_image = Image.open(val_frame_dat['image_folder'][im_num] + left_image_name)

        #give conditions if left or right image does not exist
        if(left_image_name[:-8]!=val_frame_dat['image_name'][im_num][:-8]):
            left_image = right_image
        if(right_image_name[:-8]!=val_frame_dat['image_name'][im_num][:-8]):
        ...
        diff_im_final[:,0:buffer,0:diff_im_final.shape[2]] = 0
        diff_im_final[:,diff_im_final.shape[1]-buffer:diff_im_final.shape[1],0:diff_im_final.shape[2]] = 0
