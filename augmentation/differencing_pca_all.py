...
parser.add_argument("-pl","--pca_lead_in", help="How many images to use to build the pca model",type=int,default=100)
...
parser.add_argument("-p","--pca", help="Whether to undergo PCA with whitening on the three RGB frames to reduce to two RGB frames or pca on everything to reduce to three frames. Defaults to all.",default="all")
...
for im_num in im_num_list:
    ...
    if(red_layer==True):
        ...
        if(do_pca=="two"):
            ...
            if(im_num==0):
            ...
            elif(im_num<pca_lead_in and im_num!=0):
                #continue to concatenate, up until we obtain pca_lead_in images
                im_matrix_all = np.concatenate((im_matrix_all,im_matrix))
            elif(im_num==pca_lead_in):
                #fit our concatenated image matrix to a pca model
                pca = PCA(n_components=3,whiten=True)
                pca.fit(im_matrix_all)
                
                ###apply the pca model to this im_num:
                ...
                #now re obtain the old images we used to train: 
                pca_im_nums = list(range(0,pca_lead_in))
                
                for pca_im_num in pca_im_nums:
                    im_matrix_t = im_matrix_all[pca_im_num*im_height*im_width:(1+pca_im_num)*im_height * im_width,:]
                    reduced_matrix = pca.transform(im_matrix_t)
                    reduced_image = convert_tensor(np.reshape(reduced_matrix, (im_height,im_width, 3)))
                    #apply sigmoid transform
                    reduced_image = torch.sigmoid(reduced_image)
                    diff_im_final = reduced_image
                    ...
                    if(time_stamp_loc!=False):
                        diff_im_final[:,time_stamp_loc[0]:time_stamp_loc[1],time_stamp_loc[2]:time_stamp_loc[3]]  = 0                       
                    save_image(diff_im_final,diff_loc +"/images/"+ val_frame_dat['split'][pca_im_num] + "/" + val_frame_dat['image_name'][pca_im_num])
            else:
                reduced_matrix = pca.transform(im_matrix)
                # Reshape the reduced matrix back into a 3D tensor
                reduced_image = convert_tensor(np.reshape(reduced_matrix, (im_height,im_width, 3)))
    ...
    if((do_pca=="all" and im_num>pca_lead_in) or do_pca!="all"):
        if(time_stamp_loc!=False):
            diff_im_final[:,time_stamp_loc[0]:time_stamp_loc[1],time_stamp_loc[2]:time_stamp_loc[3]]  = 0  
