...
parser = argparse.ArgumentParser(
    description="This script performs image augmentation on images with surrounding images \
...
for im_num in im_num_list:
    ...
    # give conditions if left image does not exist
    ...
    # apply optical flow
    prvs = cv2.cvtColor(
        np.array(convert_image(img_left_tens_fixed)), cv2.COLOR_BGR2GRAY
    )
    ...
    if red_layer is True:
        ...
        if do_pca is True:
            ...
            diff_im_final = torch.stack(
                (flow_mag_img, reduced_image[0, :, :], reduced_image[1, :, :]), dim=0
            )
        else:
            diff_im_final = torch.stack(
                (flow_mag_img, img_center_tens[1, :, :], img_center_tens[2, :, :]),
                dim=0,
            )
    else:
