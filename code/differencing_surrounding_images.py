...
#argument library
import argparse
parser = argparse.ArgumentParser(description="This script performs three frame differencing images with surrounding images using absolute value, variance, \
...
parser.add_argument("-bn","--BS_num", help="Number of frames to base background subtraction average on.",default=10,type=int)
...
for im_num in im_num_list:
    ...
    if(method == "naive_BS_short" or method == "diff_BS_short" or method == "diff_BS"):
        ...
        background_image_names = os.listdir(val_frame_dat['image_folder'][im_num][:-6]+'empty/')
        background_image_names_rand_sample = random.sample(background_image_names,BS_num)

        
        for rand_img_name in background_image_names_rand_sample:
            
            frame_BS_tens = convert_tensor(Image.open(val_frame_dat['image_folder'][im_num][:-6]+'empty/'+rand_img_name))
    ...
    if(method=="abs"):
        diff_im = upsilon*0.5*(abs(img_center_tens-img_right_tens_fixed) + abs(img_center_tens-img_left_tens_fixed))
